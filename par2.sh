#!/bin/bash
# This script is created to recursivly into folders detect files and create Par2 files to protect data 
...
# Start loop bsaed on tree output
tree -dfi | while read line; do
	...
	if [[ "$root$dir" == "/mnt/luke/Documents/music" ]] && (($counter > 2)) # Detecting if the loop is comepleted by waiting for the blank line from tree
	then
		exit 1	
	fi
	cd "$root$dir" 
	...
	then
		...
		then # if par2 exists then runa  repair
			pwd
	...
	let counter=counter+1 # loop counter so the script dosen't stop on the first directory
done
